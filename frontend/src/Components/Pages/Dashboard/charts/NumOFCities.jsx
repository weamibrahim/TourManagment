    ...
    const fetchBookingsData = async () => {
      try {
        const response = await axios.get(
          "https://tour-managment-three.vercel.app/api/Booking/num-of-cities",
          {
            headers: {
        ...
        if (data && data.booking && Array.isArray(data.booking)) {
          ...
          const months = Array.from(
            new Set(dataOfBooking.map((d) => d._id.month))
          ).sort();
          const cities = Array.from(
            new Set(dataOfBooking.map((d) => d._id.name))
          );
          ...
          const datasets = cities.map((city) => {
            ...
            const cityData = months.map((month) => {
              const record = dataOfBooking.find(
                (d) => d._id.name === city && d._id.month === month
              );
