...
function Search() {
  ...
  const searchHandler = async () => {
    ...
    const res = await fetch(
      `https://tour-managment-three.vercel.app/api/tour/search?city=${city}&distance=${price}&maxGroupSize=${maxGroup}`
    );
    const data = await res.json();
    navigate(
      `/search?city=${city}&distance=${price}&maxGroupSize=${maxGroup}`,
      { state: data }
    );
  };
  return (
