...
return res.status(200).json({massage:"tour created successfully", newTour})
    }
    catch(err){
        next(err);
    }
...
const tourId= req.params.id
const tour= req.body
const updateTour= await Tour.findByIdAndUpdate(tourId,tour)

return res.status(200).json({massage:"tour updated successfully",updateTour})  
    }
    catch(err){
        next(err);
    }
